 ...
 * SOUPS rewards from providing liquidity are farmed every few minutes, sold and split 50/50. 
...
contract StrategyCakeLP is Ownable, Pausable {
     ...
     * {soups} - Token generated by staking our funds. In this case it's the soups token.
    ...
    // Changed to soups address
    address constant public soups = address(0x69f27e70e820197a6e495219d9ac34c8c6da7eee);
    address constant public bifi = address(0xCa3F508B8e4Dd382eE878A314789373D80A5190A);
    
    //SOUPS-BNB LP
    address constant public lpPair = address(0x6304ae062c6bdf3d24ac86374c7019a025443247);
    
    //SOUPS
    address constant public lpToken0 = address(0x69f27e70e820197a6e495219d9ac34c8c6da7eee);
    
    //BNB
    address constant public lpToken1 = address(0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c);
    ...
    //Personal adress
    address constant public strategist = address(0x7Dc3891397F5019B60D9F1ea78c100FD7843Fc60);
     ...
     * {cakeToWbnbRoute} - Route we take to get from {soups} into {wbnb}.
     * {wbnbToBifiRoute} - Route we take to get from {wbnb} into {bifi}.
     * {cakeToLp0Route} - Route we take to get from {soups} into {lpToken0}.
     * {cakeToLp1Route} - Route we take to get from {soups} into {lpToken1}.
     */
    address[] public cakeToWbnbRoute = [soups, wbnb];
    address[] public wbnbToBifiRoute = [wbnb, bifi];
    address[] public cakeToLp0Route = [soups, lpToken0];
    address[] public cakeToLp1Route = [soups, lpToken1];
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        if (lpToken0 == wbnb) {
            cakeToLp0Route = [soups, wbnb];
        } else if (lpToken0 != soups) {
            cakeToLp0Route = [soups, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            cakeToLp1Route = [soups, wbnb];
        } else if (lpToken1 != soups) {
            cakeToLp1Route = [soups, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(masterchef, uint(-1));
        IERC20(soups).safeApprove(unirouter, uint(-1));
     ...
     * It deposits {lpPair} in the MasterChef to farm {soups}
     */
     ...
     * 3. It swaps the {soups} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(soups).balanceOf(address(this)).mul(45).div(1000);
    ...
    /**
     * @dev Swaps {soups} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 cakeHalf = IERC20(soups).balanceOf(address(this)).div(2);
    ...
    function pause() public onlyOwner {
        ...
        IERC20(soups).safeApprove(unirouter, 0);
